...
void	init_txters(t_textr **txtr, t_all *all)
{
	...
	while (all->map[go.i])
	{
		all->map[go.i] = remove_char(all->map[go.i], all);
...
int	all_in_one(t_all **all, t_textr **txtr, char **argv, t_garbage **garb)
{
	...
	(*all)->garb = (*garb);
	(*txtr)->garb = (*garb);
	ft_lst_add_back(&(*all)->garb, ft_lst_new((*txtr)));
	ft_lst_add_back(&(*all)->garb, ft_lst_new((*all)));
	ft_lst_add_back(&(*all)->garb, ft_lst_new(((*all)->txt)));
	if ((*all)->fd == -1)
	{
		printf("unvalid file\nERROR ");
		exit(1);
