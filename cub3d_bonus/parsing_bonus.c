...
void	make_duble(t_textr *txtr, t_all *all)
{
	...
	txtr->s[0] = txtr->no[1];
	ft_lst_add_back(&all->garb, ft_lst_new(txtr->s[0]));
	txtr->s[1] = txtr->so[1];
	ft_lst_add_back(&all->garb, ft_lst_new(txtr->s[1]));
	txtr->s[2] = txtr->we[1];
	ft_lst_add_back(&all->garb, ft_lst_new(txtr->s[2]));
	txtr->s[3] = txtr->ea[1];
	ft_lst_add_back(&all->garb, ft_lst_new(txtr->s[3]));
	txtr->s[4] = txtr->dor[1];
	ft_lst_add_back(&all->garb, ft_lst_new(txtr->s[4]));
	txtr->s[5] = NULL;
}
